...
public class Game extends JFrame implements ActionListener {
    ...
    Game() {
        JFrame gameFrame = new JFrame("IT211 Project 4 - BlackJack");
        gameFrame.setIconImage(Toolkit.getDefaultToolkit().getImage("BJcards/10.png"));
    ...
    private void hitPlayer() {
        ...
        playerPanel.add(new JLabel(new ImageIcon("BJcards/" + newCard.toString())));
        playerPanel.updateUI();
    }

    private void hitDealerDown() {
        Card newCard = dealer.dealTo(deck.dealFrom());
        dealerPanel.add(new JLabel(new ImageIcon("BJcards/b2fv.png")));
        dealerPanel.updateUI();
    }

    private void hitDealer() {
        Card newCard = dealer.dealTo(deck.dealFrom());
        dealerPanel.add(new JLabel(new ImageIcon("BJcards/" + newCard.toString())));
        dealerPanel.updateUI();
    }
    ...
    private void checkWinner() {
        ...
        for (int i = 0; i < dealer.inHand(); i++) {
            dealerPanel.add(new JLabel(new ImageIcon("BJcards/" + dealer.cards[i].toString())));
        }
        if (player.value() > 21) {
